package com.eim.utility.contribution.api;
...
import com.eim.utility.contribution.model.Security;
 ...
 *
 */
public abstract class ValuationContentExtractor {

	/**
	 * The logging utility
	 */
	private Logger logger = Logger.getLogger(ValuationContentExtractor.class);
	...
	/**
	 * Returns the filename of the AUMs extraction query
	 * @return The filename
	 */
	public abstract String getAllAUMSQueryName();
	
	/**
	 * Returns the filename of the AUMs extraction query within a given range of dates
	 * @return The filename
	 */
	public abstract String getAllAUMSWithinRangeQueryName();
	
	/**
	 * Returns the filename of the Portfolio Cashflows extraction query
	 * @return The filename
	 */
	public abstract String getAllCashflowsQueryName();
	/**
	 * Returns the filename of the Portfolio Cashflows extraction query within a given range of dates
	 * @return The filename
	 */
	public abstract String getAllCashflowsWithinRangeQueryName();
	
	/**
	 * Returns the filename of the Underlying AUMs extraction query
	 * @return The filename
	 */
	public abstract String getAllUnderlyingAUMSQueryName();
	/**
	 * Returns the filename of the Underlying AUMs extraction query within a given range of dates
	 * @return The filename
	 */
	public abstract String getAllUnderlyingAUMSWithinRangeQueryName();
	
	
	/**
	 * Returns the filename of the Portfolio's underlying cashflows extraction query.<BR/>
	 * This includes: Series rollover, equalization factor, investments, redemptions ...
	 * @return The filename
	 */
	public abstract String getAllUnderlyingCashflowsQueryName();
	
	/**
	 * Returns the filename of the Portfolio's underlying cashflows extraction query within a given range of dates.<BR/>
	 * This includes: Series rollover, equalization factor, investments, redemptions ...
	 * @return The filename
	 */
	public abstract String getAllUnderlyingCashflowsWithinRangeQueryName();
	...
	public Collection<Evaluation> getPortfolioEvaluationsWithinPeriod(Long id, Date fromDate, Date toDate) {
		...
		logger.info("Get evaluations for portfolio with id [" + id.toString() + "] between " + fromDate.toString() + " and " + toDate.toString() + ", using " + queryName);
	...
	public Collection<Position> getEvaluationSecurities(Long id, Long refID) {
		logger.info("Get securities for evaluation with id [" + id.toString() + "] and reference evaluation with id [" + refID.toString() + "], using " + getEvaluationSecuritiesQueryName());
		...
		try {
			// Make SQL query
			PreparedStatement preparedStatement = connection.prepareStatement(QueriesProvider.getQuery(getEvaluationSecuritiesQueryName()));
			preparedStatement.setLong(1, id);
			preparedStatement.setLong(2, id);
	 ...
	 * @return The list of securities
	 */
	public Collection<Switch> getPortfolioSwitches(Long portfolioID, Date fromDate, Date toDate) {
		// Get name of query to be used
		...
		try {
			...
			preparedStatement.setDate(3, new java.sql.Date(toDate.getTime()));
			preparedStatement.setLong(4, portfolioID);
	 ...
	 * @return The list of securities
	 */
	public Collection<Operation> getPortfolioOperations(Long portfolioID, Date fromDate, Date toDate) {
		// Get name of query to be used
	 ...
	 * @return The list of securities
	 */
	public void getPortfolioPenalties(Long portfolioID, Date fromDate, Date toDate, Collection<Position> securities) {
		// Get name of query to be used
	...
	/**
	 * Default utility method that extracts all the AUM of the given portolio.
	 * @param id Portfolio id
	 * @param frequency Frequency
	 * @return The list of AUMs
	 */
	public Collection<AUMToken> getPortfolioAUMS(String id, ComputationFrequency frequency) {
		logger.info("AUMS content with id [" + id + "] and frequency " + frequency  + " using " + getAllAUMSQueryName());
		ArrayList<AUMToken> results = null;
		Connection connection = QueriesProvider.getConnection("source");
		try {
			PreparedStatement prepareStatement = connection.prepareStatement(QueriesProvider.getQuery(getAllAUMSQueryName()));
			prepareStatement.setString(1, id);
			results = readDatabaseAUMSData(new ArrayList<AUMToken>(),prepareStatement,frequency);
			prepareStatement.close();
			connection.close();
		} catch (SQLException e) {
			logger.error("A SQL error occured during AUMS content extraction!",e);
		}
	 ...
	 * @return The list of AUMs
	 */
	public Collection<AUMToken> getPortfolioAUMS(String id, Date fromDate, Date toDate, ComputationFrequency frequency) {
		logger.info("AUMS content with id [" + id + "] and frequency " + frequency  + " between " + DateFormatter.toInputString(fromDate) + " to "  + DateFormatter.toInputString(toDate) + " using " + getAllAUMSWithinRangeQueryName());
		ArrayList<AUMToken> results = null;
		Connection connection = QueriesProvider.getConnection("source");
		try {
			PreparedStatement prepareStatement = connection.prepareStatement(QueriesProvider.getQuery(getAllAUMSWithinRangeQueryName()));
			prepareStatement.setString(1, id);
	...
	/**
	 * Default utility method that extracts all the cashflows of the given portolio.
	 * @param id Portfolio id
	 * @param fromDate Start date
	 * @param toDate End date
	 * @param frequency Frequency
	 * @return The list of cashflows
	 */
	public Collection<CashflowsToken> getPortfolioCashflows(String id, Date fromDate, Date toDate, ComputationFrequency frequency) {
		logger.info("CASHFLOWS content with id [" + id + "] and frequency " + frequency  + " between " + DateFormatter.toInputString(fromDate) + " to "  + DateFormatter.toInputString(toDate) + " using " + getAllAUMSQueryName());
		ArrayList<CashflowsToken> results = null;
		Connection connection = QueriesProvider.getConnection("source");
		try {
			PreparedStatement prepareStatement = connection.prepareStatement(QueriesProvider.getQuery(getAllCashflowsWithinRangeQueryName()));
			prepareStatement.setString(1, id);
			prepareStatement.setDate(2, new java.sql.Date(fromDate.getTime()));
			prepareStatement.setDate(3, new java.sql.Date(toDate.getTime()));
			results = readDatabaseCashflowsData(new ArrayList<CashflowsToken>(),prepareStatement,frequency);
			prepareStatement.close();
			connection.close();
		} catch (SQLException e) {
			logger.error("A SQL error occured during CASHFLOWS content extraction!",e);
		}
	...
	/**
	 * Default utility method that extracts all the underlying aums of the given portolio.
	 * @param id Portfolio id
	 * @param fromDate Start date
	 * @param toDate End date
	 * @param frequency Frequency
	 * @return
	 */
	public HashMap<Long, ArrayList<AUMToken>> getPortfolioUnderlyingFundsAUMS(String id, Date fromDate, Date toDate, ComputationFrequency frequency) {
		logger.info("UNDERLYNG AUMS content of id [" + id + "] and frequency " + frequency  + " between " + DateFormatter.toInputString(fromDate) + " to "  + DateFormatter.toInputString(toDate) + " using " + getAllAUMSQueryName());
		HashMap<Long, ArrayList<AUMToken>> results = null;
		
		Connection connection = QueriesProvider.getConnection("source");
		try {
			PreparedStatement prepareStatement = connection.prepareStatement(QueriesProvider.getQuery(getAllUnderlyingAUMSWithinRangeQueryName()));
			prepareStatement.setString(1, id);
			prepareStatement.setDate(2, new java.sql.Date(fromDate.getTime()));
			prepareStatement.setDate(3, new java.sql.Date(toDate.getTime()));
			results = readDatabaseUnderlyingAUMSData(new HashMap<Long, ArrayList<AUMToken>>(),prepareStatement,frequency);
			prepareStatement.close();
			connection.close();
		} catch (SQLException e) {
			logger.error("A SQL error occured during CASHFLOWS content extraction!",e);
	...
	/**
	 * Default utility method that extracts all the underlying cashflows of the given portolio.
	 * @param id Portfolio id
	 * @param fromDate Start date
	 * @param toDate End date
	 * @param frequency Frequency
	 * @return
	 */
	public HashMap<Long, ArrayList<CashflowsToken>> getPortfolioUnderlyingFundsCashflows(String id, Date fromDate, Date toDate, ComputationFrequency frequency) {
		logger.info("UNDERLYNG CASHFLOWS content of id [" + id + "] and frequency " + frequency  + " between " + DateFormatter.toInputString(fromDate) + " to "  + DateFormatter.toInputString(toDate) + " using " + getAllAUMSQueryName());
		HashMap<Long, ArrayList<CashflowsToken>> results = null;
		
		Connection connection = QueriesProvider.getConnection("source");
		try {
			PreparedStatement prepareStatement = connection.prepareStatement(QueriesProvider.getQuery(getAllUnderlyingCashflowsWithinRangeQueryName()));
			prepareStatement.setString(1, id);
			prepareStatement.setDate(2, new java.sql.Date(fromDate.getTime()));
			prepareStatement.setDate(3, new java.sql.Date(toDate.getTime()));
			results = readDatabaseUnderlyingCashflowsData(new HashMap<Long, ArrayList<CashflowsToken>>(),prepareStatement,frequency);
			prepareStatement.close();
			connection.close();
		} catch (SQLException e) {
			logger.error("A SQL error occured during CASHFLOWS content extraction!",e);
	...
	protected abstract HashMap<Long, ArrayList<CashflowsToken>> readDatabaseUnderlyingCashflowsData(HashMap<Long, ArrayList<CashflowsToken>> cashflows, PreparedStatement statement, ComputationFrequency frequency);
	
	protected abstract ArrayList<CashflowsToken> readDatabaseCashflowsData(ArrayList<CashflowsToken> cashflows, PreparedStatement statement, ComputationFrequency frequency) throws SQLException;
	
	protected abstract ArrayList<AUMToken> readDatabaseAUMSData(ArrayList<AUMToken> aums,PreparedStatement statement, ComputationFrequency frequency) throws SQLException;
	
	protected abstract HashMap<Long, ArrayList<AUMToken>> readDatabaseUnderlyingAUMSData(HashMap<Long, ArrayList<AUMToken>> aums,PreparedStatement statement, ComputationFrequency frequency) throws SQLException;
